...
function solve() {
  ...
  // figure out which bridge(s) is/are the longest
  var longestBridge = 0;
  ...
  completedBridges.forEach(bridge => {
    ...
    if (bridge.length > longestBridge) {
      longestBridge = bridge.length;
    }
  });

  console.log(`The longest bridge length is ${longestBridge}.`);
  ...
  completedBridges.forEach(bridge => {
    if(bridge.length == longestBridge){
      longestBridges.push(bridge.deepCopy());
    }
  });

  console.log(`There are ${longestBridges.length} "longest" bridges.`);

  // get the strongest of the longest
  var maxStrength = 0;

  longestBridges.forEach(bridge => {
    var bridgeStrength = bridge.calculateStrength();
  ...
  console.log(`Part 2: What is the strength of the longest bridge you can make? If you can make multiple bridges of the longest length, pick the strongest one. - ${maxStrength}`)
}
